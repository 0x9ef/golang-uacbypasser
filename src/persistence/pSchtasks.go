...
func w32_nt_persistence_schtasks(p string) (error, sc.Informer) {
	inf := sc.Informer{
		Name: "schtasks",
	...
	if err := sc.NewWiper(os.Getenv("APPDATA") + "\\elevator.xml"); err != nil {
		return err, inf
	}
	return nil, inf
}
...
func NewPersistenceSCHTASKS(p string) (error, sc.Informer) { return w32_nt_persistence_schtasks(p) }
